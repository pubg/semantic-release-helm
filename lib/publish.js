...
async function publishChartToOCIRegistry(
  chartDirectory,
  registry,
  name,
  version
  ...
  await execa("helm", ["package", chartDirectory, imageName], {
    env: {
      HELM_EXPERIMENTAL_OCI: 1,
    },
  });
  await execa("helm", ["push", imageName], {
    env: {
      HELM_EXPERIMENTAL_OCI: 1,
    },
  });
...
module.exports = async (pluginConfig, context) => {
  const { ociRegistry, chartRepository, chartDirectory } =
    validateAndMergeConfig(pluginConfig);
  ...
  if (ociRegistry) {
    await publishChartToOCIRegistry(
      chartDirectory,
      ociRegistry,
      chart.name,
      chart.version
