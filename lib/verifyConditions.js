...
const { validateAndMergeConfig, escapeShell } = require('./utils');
...
async function loginChartRepository(repository, username, password) {
    try {
        ...
        return await execa('helm', ['repo', 'add', 'semantic-release-helm', '--username', escapeShell(username), '--password', password, repository])
    } catch (error) {
...
async function loginOCIRegistry(repository, username, password) {
    return await execa('helm', ['registry', 'login', '--username', escapeShell(username), '--password-stdin', repository], {
        input: password,
        env: {
